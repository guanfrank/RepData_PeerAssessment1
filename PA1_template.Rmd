...
```
The max step `r maxStep$steps` happened at `r maxStep$intervalTime` of the day
...
meanImputedStep <- mean(stepsImputedDaily)
medianImputedStep <- median(stepsImputedDaily)
```
